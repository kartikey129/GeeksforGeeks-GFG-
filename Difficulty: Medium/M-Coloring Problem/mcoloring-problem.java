...
class Solution {
    ...
    boolean solve(int node, List<List<Integer>> adj, int n, int m, int[] color) {
        if (node == n) return true;

        for (int i = 1; i <= m; i++) {
            if (isSafe(node, adj, n, i, color)) {
                color[node] = i;
                if (solve(node + 1, adj, n, m, color)) return true;
    ...
    boolean isSafe(int i, List<List<Integer>> adj, int n, int col, int[] color) {
